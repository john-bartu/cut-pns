...
int main(int argc, char **argv) {
    ...
    if (access("./public.key", F_OK) != 0) {
        fprintf(stderr, "./public.key not exists\n");
        return -1;
    }

    if (access("./private.key", F_OK) != 0) {
        fprintf(stderr, "./private.key not exists\n");
        return -1;
    }
...
int authenticate_user(LIBSSH2_SESSION *session, struct connection_data *cd) {
    ...
    for (;;) {
        ...
        if (libssh2_userauth_publickey_fromfile(session, cd->username, "./public.key", "./private.key", password) ==
            0) {
